...
class OnboardingContentARootView: NiblessView {
    
    let imageView: UIImageView = {
        let imageView = UIImageView(image: UIImage(named: "logo-standard-large"))
        return imageView
    }()
    
    let headingLabel: UILabel = {
        let label = UILabel()
        label.text = "Welcome to Quiet Web"
    ...
    let bodyLabel: UILabel = {
        let label = UILabel()
        label.text = "We’ll help you block distractions in Safari, so you can spend time on what matters most."
